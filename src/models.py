...
class StoryGraph:
    ...
    # ================================================================
    # 转换为 CharacterGraph（向后兼容）
    # ================================================================

    def to_character_graph(self):
        """提取人物子图为 CharacterGraph。"""
        cg = CharacterGraph(last_updated_chapter=self.last_updated_chapter)
        for pn in self.person_nodes:
            cg._add_node(pn)
        for re in self.relationship_edges:
            cg.add_edge(re)
        cg.timeline = list(self.timeline)
        return cg
    ...
    def from_dict(cls, d: dict) -> "StoryGraph":
        # 检测旧格式
        if d.get("_schema_version", 0) < 2:
            return cls._from_old_format(d)
    ...
    @classmethod
    def _from_old_format(cls, d: dict) -> "StoryGraph":
        """从旧 CharacterGraph 格式迁移。"""
        graph = cls(last_updated_chapter=d.get("last_updated_chapter", 0))
        for nd in d.get("nodes", []):
            node = CharacterNode.from_dict(nd)
        ...
        for ed in d.get("edges", []):
            edge = RelationshipEdge.from_dict(ed)
            # 确保两端节点存在
            if graph.get_person_node(edge.from_char) is None:
                graph._ensure_node_exists(
                    graph._key("person", edge.from_char), "person", edge.from_char)
            if graph.get_person_node(edge.to_char) is None:
                graph._ensure_node_exists(
                    graph._key("person", edge.to_char), "person", edge.to_char)
...
class Novel:
    ...
    @property
    def character_graph(self) -> Optional[CharacterGraph]:
        """向后兼容：从 StoryGraph 中提取人物子图。"""
        if self.story_graph is None:
            return None
        return self.story_graph.to_character_graph()
