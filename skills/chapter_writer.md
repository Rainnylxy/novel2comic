...
1. **lookup_character(name)** — 查询角色档案。已缓存在工作记忆中的角色直接复用，不要重复查询
...
3. **身份一致**: lookup_character 返回的"身份"字段包含角色的职业/职位（如警察、医生、学生），角色的外貌着装、行为举止必须与其职业身份一致。例如：警察出现场不可能穿西装，刑警穿便衣或警服
4. **逐节写作**: 根据规划中的section的target_fragments 要求，撰写本节内容，不要一口气写完整个章节
5. **片段交替**: 对话（dialogue）和动作（action）交替推进，不要连续输出太长的旁白（narration）
6. **文风一致**: 模仿文风 Profile 的笔法和节奏
7. **角色准确**: dialogue/action/inner_thought 的 character 字段必须用原文中的准确角色名
